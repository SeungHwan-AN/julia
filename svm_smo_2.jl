...
function objective(alpha)
    result = sum(alpha)
    for i in 1:n
        result = result -sum(y[i] .* y .* (X * X[i, :]) .* alpha[i] .* alpha) ./ 2
    end
    return result
end
...
####################################################
# output function
function output_function(i, alpha)
    return sum(alpha .* y .* (X * X[i, :])) - b
end

#######################################################
# takestep
function takestep(i1, i2, a2, y2, E2, alpha)
    if (i1 == i2) 
        return false
    end
    a1 = alpha[i1]
    y1 = y[i1]
    E1 = output_function(i1, alpha) - y1
    s = y1 * y2
    if (s != 1)
        L = max(0, a2 - a1)
        H = min(C, C + a2 - a1)
    ...
    if (L == H)
        # println("L and H are same values!")
        return false
    end
    ...
    # println("new_a2: ", new_a2)

    if (abs(new_a2 - a2) < eps * (new_a2 + a2 + eps))
        # println("new_a2 changed too small...")
        return false
    end

    new_a1 = a1 + s * (a2 - new_a2)

    # println("new_a1: ", new_a1)
...
####################################################
# examine_example
function examine_example(i2, alpha)
    y2 = y[i2]
    a2 = alpha[i2]
    E2 = output_function(i2, alpha) - y2
    r2 = E2 * y2 
    if (((r2 < -tol) & (a2 < C)) | ((r2 > tol) & (a2 > 0)))
        if (sum(non_bound_alpha) > 1)
            i1 = second_choice(E2)
            if takestep(i1, i2, y2, a2, E2, alpha)
                return 1
            end
        end
...
#     if (examine_all)
#         for i in 1:n
#             num_changed = num_changed + examine_example(i, alpha)
#         end
#     else
...
for iter in 1:maxiter
    if (examine_all)
        for i in 1:n
            global num_changed = num_changed + examine_example(i, alpha)
        end
    else
        for i in findall(!iszero, non_bound_alpha)
            global num_changed = num_changed + examine_example(i, alpha)
